<script lang="ts">
	...
	$: rankingsCollection = $entities?.value.find((e) => e.get().path === rankingsPath);

	$: ({space_id, directory_id} = $space);
	$: ({actor_id} = $actor);

	$: if ($query?.status === 'success' && !rankingsCollection) {
		void initRankingsCollection(space_id, directory_id, actor_id, rankingsPath);
	}
	const initRankingsCollection = async (
		space_id: SpaceId,
	...
	$: orderedItems = $rankingsCollection?.data.orderedItems;
	$: orderedItems && void assignOrderedEntities();
	const assignOrderedEntities = async (): Promise<void> => {
		orderedEntities = await loadOrderedEntities($rankingsCollection!, $actor.actor_id, ui, actions);
	};
	...
	const createList = async () => {
		...
		await actions.CreateEntity({
			...
			ties: [{source_id: $rankingsCollection.entity_id}],
		});
		creating = false;
	};
...
<div class="todo">
	...
	{#if query && rankingsCollection && $rankingsCollection && orderedEntities}
		<TodoItems
			{actor}
			parentList={rankingsCollection}
		...
		<div class="row">
			<TextInput
				...
				autofocus={true}
				placeholder="> new ranking"
			/>
			<Pending_Button pending={creating} on:click={createList}>create new ranking</Pending_Button>
		</div>
	{:else}
		<Pending_Animation />
	{/if}

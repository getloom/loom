...
test__CommunityRepo('updateSettings', async ({repos, random}) => {
	...
	unwrap(await repos.community.updateSettings(community.community_id, newSettings));
	assert.equal(
		unwrap(await repos.community.findById(community.community_id))?.settings,
		newSettings,
	);
});
